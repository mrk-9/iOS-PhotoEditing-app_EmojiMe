...
class ObjectViewController: UIViewController {
    ...
    @IBOutlet weak var ObjLibraryCollectionView: UICollectionView!
    var pngFiles: [URL] = []
    ...
    override func viewDidLoad() {
        ...
        do {
            ...
            let pngFiles = directoryContents.filter{ $0.pathExtension == "jpeg" && $0.absoluteString.contains("Object") }
            pngFileNames = pngFiles.map{ $0.deletingPathExtension().lastPathComponent }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.ObjLibraryCollectionView.reloadData()
    }
...
extension ObjectViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if (collectionView == self.ObjectCollectionView) {
            return 46
        }
        else {
    ...
    func showOptionMenu(optionType: Bool, shareImage: UIImage, deleteIndex: Int) {
        ...
        sharingItems.append(shareImage)
        ...
        if (optionType == true) {
            let delete = UIAlertAction(title: "Delete", style: .default, handler: {(UIAlertAction) -> Void in
                ...
                let ok = UIKit.UIAlertAction(title: "Ok", style: UIAlertActionStyle.default, handler: {(UIAlertAction) -> Void in
                    ...
                    self.ObjLibraryCollectionView.reloadData()
                    self.ObjLibraryCollectionView.layoutIfNeeded()
                })
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if (collectionView == self.ObjectCollectionView) {
            ...
            cell.objectImage.image = UIImage(named: "object" + "\(indexPath.row+1)" + ".png")
            self.preSetFiles.append("image" + "\(indexPath.row+1)" + ".png")
            return cell
            
        } else {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "ObjLibraryCollectionViewCell", for: indexPath) as! ObjLibraryCollectionViewCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if (collectionView == self.ObjectCollectionView) {
            let cell = collectionView.cellForItem(at: indexPath) as! ObjectCollectionViewCell
            let img: UIImage = self.addShadow(image: cell.objectImage.image!)
            ...
            let cell = collectionView.cellForItem(at: indexPath) as! ObjLibraryCollectionViewCell
